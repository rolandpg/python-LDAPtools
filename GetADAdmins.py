...
def CollectADComputers(Domain_target):
    ...
    with open('C:\\Users\\patrick.roland\\Documents\\new_adminList{}.csv'.format(Domain_target), 'w') as txtfile: #where the txtfile needs to land
        for x in range(0, len(ADReader)):
            txtfile.write(str(ADReader[x].cn) +','+ str(ADReader[x].description)+','+str(ADReader[x].name)+','+str(ADReader[x].mail)+'\n' )
        '''
